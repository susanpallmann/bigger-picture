...
function updatePage(prevScroll, currentScroll) {
    ...
    $(".animal:visible").each(function() {
      ...
      $(this).find(".background").css("transform", "translateZ(" + 0.05*offsetAmount + ")");
      $(this).find(".midback").css("transform", "translateZ(" + 0.025*offsetAmount + ")");
      $(this).find(".midground").css("transform", "translateZ(" + 0.025*(spaceHeight - offsetAmount) + ")");
      $(this).find(".foreground").css("transform", "translateZ(" + 0.05*(spaceHeight - offsetAmount) + ")");
    });
    scrollValue = currentScroll;
}
