...
function updatePageOnScroll(prevScroll, currentScroll) {
    ...
    $(".animal").each(function() {
      if (isScrolledIntoView($(this))) {
        ...
        $(this).find(".background").css("transform", "translateY(" + (0.5*($(this).find(".background").outerHeight()-spaceHeight)*(offsetAmount/spaceHeight)) + "px)");
        $(this).find(".midback").css("transform", "translateY(" + (0.5*($(this).find(".midback").outerHeight()-spaceHeight)*(offsetAmount/spaceHeight)) + "px)");
        $(this).find(".midground").css("transform", "translateY(" + (-0.5*($(this).find(".midground").outerHeight()-spaceHeight)*(offsetAmount/spaceHeight)) + "px)");
        $(this).find(".foreground").css("transform", "translateY(" + (-0.5*($(this).find(".foreground").outerHeight()-spaceHeight)*(offsetAmount/spaceHeight)) + "px)");
      }
    });
